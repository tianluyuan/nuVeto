...
def PlotNuMuConventionalPassingFraction(cos_theta, particle=14, pmodel=(pm.HillasGaisser2012, 'H3a'), hadr='SIBYLL2.3c', prpl='step_1'):
    """ plot passing fraction for the conventional component
    """
    enu_grid = np.logspace(3, 10, 20)
    enu_grid_fine = np.logspace(3, 10, 1000)
    pnm = [sv.passing(enu, cos_theta, kind = 'conv_numu') for enu in enu_grid]
